        ...
        <at-breadcrumb separator=">">
            ...
            <at-breadcrumb-item
                :to="{
                    ...
                    params: { subsystemId: category.distributive.subsystem }
                }"
            >Дистрибутив подсистемы: {{ distributive.subsystem.name }}</at-breadcrumb-item>
            <at-breadcrumb-item :to="{ name: 'admin-distributives-categories-distributiveId', params: { distributiveId: distributive.id } }">Категории дистрибутива: {{ distributive.name }} </at-breadcrumb-item>
...
<script>
    ...
    export default {
        async asyncData({store, params}) {
            ...
            const distributive = await store.dispatch(`distributives/${actionNames.GET_ITEM}`, { id: category.distributive.id })
            return { category, distributive }
        },
