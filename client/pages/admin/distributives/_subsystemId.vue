<template>
    <div>
        <at-breadcrumb separator=">">
            ...
            <at-breadcrumb-item>Дистрибутивы подсистемы {{ subsystem.name }}</at-breadcrumb-item>
        </at-breadcrumb>
        <h2>{{ title }}</h2>
...
<script>
    ...
    const converter = new DataModelConverter(Distributive, {
        schema: {
            order: ['name','server_bd', 'db_ms','bd_name','server_app', 'server_web','server_update', 'orderField'],
            hidden: ['id']
        },
        columns: {
    ...
    function makeColumns(converter) {
        ...
        return tableColumnsBuilder
            ...
            .addOperation({
                title: 'Категории',
                onClick: (params) => {
                    this.$router.push(`/admin/distributives/categories/${params.item.id}`)
                }
            })
            .build()
    }
    ...
    export default {
        ...
        computed: {
            ...
            title() {
                return `Дистрибутивы подсистемы ${this.subsystem.name}`
            }
        },
        layout: 'admin',
        components: {
        ...
        data() {
            return {
                ...
                selectionParent:[],
                modalTitles: {
                    create: 'Создание дистрибутива',
                    edit: 'Изменение дистрибутива'
                },
            }
        },
        methods: {
